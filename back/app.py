...
# Register custom JSON provider for ObjectId serialization
app.json_provider_class = MongoJSONProvider
app.json = MongoJSONProvider(app)

# Replace your current CORS setup with this in app.py
CORS(
    app,
...
def list_users():
    ...
    for u in users:
        u["_id"] = str(u["_id"])
        login_id = oid(u["usertype"])
        u["usertype"] = str(login_id)

        # Join Donor Info to get Blood Group and Age
        donor_info = mongo.db.donors.find_one({"userId": oid(u["_id"])})
        if donor_info:
            u["details"] = {
...
def list_hospitals():
    ...
    for h in hospitals:
        h["_id"] = str(h["_id"])
        h["loginId"] = str(h["loginId"])
        
        # Join Login details if needed
        login = mongo.find_one("login", {"_id": oid(h["loginId"])})
        if login:
...
def list_blood_requests():
    ...
    for r in requests:
        r["_id"] = str(r["_id"])
        r["hospitalLoginId"] = str(r["hospitalLoginId"])

        # Join hospital details
        hospital = mongo.find_one("hospitals", {
            "loginId": oid(r["hospitalLoginId"])
        })

        if hospital:
            hospital["_id"] = str(hospital["_id"])
            hospital["loginId"] = str(hospital["loginId"])
...
def get_hospital_stats(hospital_login_id):
    try:
        ...
        requests_cursor = mongo.db.requesthospital.find({"hospitalLoginId": h_oid})
        ...
        # Get Recent Activity (FIXED: showing actual data)
        recent_requests = list(mongo.db.requesthospital.find({"hospitalLoginId": h_oid})
